...
function DownChevron({ open }: { open: boolean }) {
  return (
    ...
    >
      <path fillRule="evenodd" clipRule="evenodd" d="M10 0.838357L5 6L0 0.838357L0.812102 0L5 4.32329L9.1879 0L10 0.838357Z" fill={NAVY} />
    </svg>
  );
}
...
const SHOP_BY_ACTIVITY_ITEMS = [
  "Shop All", "Yoga & Studio", "Run & Train", "Tennis Golf & Pickleball",
  "Everyday Lifestyle", "Travel & Commute", "Hiking", "Home Workout", "Walking", "Soccer",
];

const SHOP_BY_FABRICATION_ITEMS = [
  "Shop All", "New! Powerchill", "Powersoft Boundless", "Bounce Fleece", "StudioSmooth",
  "Powersoft", "Dynamic Fleece", "CloudMotion", "SleekTech", "CloudComfy",
  "And more!", "Other", "Other", "Other", "Other", "Other",
];

const ACCORDIONS: { label: string; items: string[] }[] = [
  { label: "Shop By Activity", items: SHOP_BY_ACTIVITY_ITEMS },
  { label: "Shop By Fabrication", items: SHOP_BY_FABRICATION_ITEMS },
  { label: "Shop for the Fam", items: ["Women", "Men", "Girls", "Boys", "Toddler Girls", "Toddler Boys", "Maternity"] },
];

function AccordionRow({ label, items, open, onToggle, stickyTop, stickyTransition }: {
  label: string; items: string[]; open: boolean; onToggle: () => void;
  stickyTop: number; stickyTransition: string;
}) {
  const panelHeight = items.length * 40;
  return (
    <div>
      <div style={{ position: "sticky", top: stickyTop, zIndex: 10, background: "#fff", borderBottom: "1px solid #cecece", transition: stickyTransition }}>
        <button onClick={onToggle} style={{ width: "100%", height: 48, background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", padding: 0 }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: NAVY, letterSpacing: 0.14, lineHeight: "20px" }}>{label}</span>
          <DownChevron open={open} />
        </button>
      </div>
      <div style={{ overflow: "hidden", maxHeight: open ? panelHeight : 0, transition: "max-height 0.28s ease" }}>
        {items.map((item) => (
          <button key={item} style={{ width: "100%", display: "flex", alignItems: "center", background: "none", border: "none", cursor: "default", padding: "12px 0", textAlign: "left" }}>
            <span style={{ fontSize: 12, fontWeight: 400, color: "#4a4a4a", letterSpacing: 0.12, lineHeight: "16px" }}>{item}</span>
          </button>
        ))}
      </div>
    </div>
...
export default function ShopAllContent({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0 }}>
      {/* White background so it covers the page beneath during transition */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "#fff" }} />

      {/* L2 top nav — Figma node 2335-19840 */}
      <div style={{ position: "absolute", top: 55, left: 0, width: 390, height: 48, background: "#fff", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "space-between", padding: 8 }}>
        {/* Left Icon */}
        <div style={{ display: "flex", alignItems: "center", padding: 8, flexShrink: 0 }}>
          <button onClick={() => onBack ? onBack() : router.back()} style={{ width: 24, height: 24, background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <svg width="9" height="16" viewBox="0 0 8.81066 15.5" fill="none" overflow="visible">
              <path d="M8.06066 0.75L1.06066 7.75L8.06066 14.75" stroke={NAVY} strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </button>
        </div>
        {/* Title */}
        <div style={{ display: "flex", flexDirection: "column", gap: 2, alignItems: "flex-start", justifyContent: "center", width: 200, flexShrink: 0 }}>
          <span style={{ fontSize: 16, fontWeight: 400, color: NAVY, fontFamily: "'ON Sans Text', sans-serif", textAlign: "center", width: "100%", lineHeight: "normal", wordBreak: "break-word" }}>
            Shop All
          </span>
        </div>
        {/* Right Icon — heart */}
      ...
      {/* Overlay search bar — eases in on scroll-up */}
      <div style={{
        position: "absolute", top: 103, left: 0, width: 390, zIndex: 15,
        transform: overlayVisible ? "translateY(0)" : "translateY(-56px)",
      ...
      <div ref={scrollRef} onScroll={handleScroll} style={{
        ...
        <SearchBar />
        {/* L1 Body — py-8, gap-16 between image and accordions (matches Figma 2335-19856) */}
        <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingTop: 8, paddingBottom: 8 }}>
          <div style={{ width: "100%", height: 260, position: "relative", overflow: "hidden" }}>
            <Image src="/images/sport/shop_all_hero.png" alt="Shop All" fill priority style={{ objectFit: "cover", objectPosition: "center top" }} sizes="390px" />
          </div>
          <div style={{ paddingLeft: 16, paddingRight: 16 }}>
            {ACCORDIONS.map(({ label, items }) => (
              <AccordionRow
                key={label} label={label} items={items}
                open={openAccordion === label} onToggle={() => toggle(label)}
                stickyTop={overlayVisible ? 56 : 0}
                stickyTransition={overlayVisible ? "top 0.28s ease-out" : "none"}
              />
            ))}
          </div>
        </div>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, width: 390, height: 85, background: "#fff", borderTop: "1px solid #cecece", display: "flex", flexDirection: "column", alignItems: "center" }}>
        <div style={{ height: 51, width: 390, display: "flex", alignItems: "center", paddingTop: 4, paddingBottom: 4 }}>
          ...
          <button style={{ width: 80, height: 43, background: "none", border: "none", cursor: "default", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }}>
            <ShirtIcon />
            <span style={{ fontSize: 10, color: NAVY, fontWeight: 400, lineHeight: 1.1 }}>Shop</span>
          </button>
          <button style={{ width: 80, height: 43, background: "none", border: "none", cursor: "default", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }}>
            <DealsIcon />
            <span style={{ fontSize: 10, color: NAVY, fontWeight: 400, lineHeight: 1.1 }}>Deals</span>
          </button>
          <button style={{ width: 80, height: 43, background: "none", border: "none", cursor: "default", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }}>
            <BagIcon />
            <span style={{ fontSize: 10, color: NAVY, fontWeight: 400, lineHeight: 1.1 }}>Bag</span>
          </button>
          <button style={{ width: 80, height: 43, background: "none", border: "none", cursor: "default", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }}>
            <AccountIcon />
            <span style={{ fontSize: 10, color: NAVY, fontWeight: 400, lineHeight: 1.1 }}>Account</span>
          </button>
        </div>
