...
import ShopAllContent from "./ShopAllContent";
...
export default function ShopAllPage() {
  ...
  return (
    <div style={{
      ...
      {/* Status bar — fixed above all transitions, zIndex 30 */}
      ...
      {/* Sport page sits underneath as the background — non-interactive */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none" }}>
        <SportContent />
      </div>

      {/* Shop All content slides over Sport, exits to the right on back */}
      <div style={{
        position: "absolute", top: 0, left: 0, right: 0, bottom: 0,
        zIndex: 25,
        transform: exiting ? "translateX(100%)" : "translateX(0)",
        transition: exiting ? "transform 0.28s ease-in" : "none",
      }}>
        <ShopAllContent onBack={handleBack} />
      </div>
    </div>
  );
}
