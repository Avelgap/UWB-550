...
// Used for sport hero cards while real assets are pending

function SportPlaceholder({ label, aspectRatio }: { label: string; aspectRatio: string }) {
  const colors: Record<string, string> = {
    "Workout & Training": "#1B3A5C",
    "Running":            "#0D2B45",
    "Yoga & Studio":      "#2A4A6B",
    "Outdoor & Travel":   "#1A3550",
    "Sports Bras & Tops": "#0F2D47",
    "Shop All":           "#05274C",
    "Kids":               "#163D60",
  };
  const bg = colors[label] || NAVY;
  return (
    <div style={{ width: "100%", aspectRatio, background: `linear-gradient(160deg, ${bg} 0%, #0a3d6b 100%)`, position: "relative", display: "flex", alignItems: "flex-end", padding: 8 }}>
      <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: 1 }}>{label}</span>
    </div>
  );
}
...
export default function SportPage() {
  ...
  // IntersectionObserver: fires the instant the inline header enters/leaves view
  useEffect(() => {
  ...
  const goToShopAll = useCallback(() => router.push("/shop/sport/shop-all"), [router]);
  ...
  return (
    ...
    >
      {/* ── FIXED HEADER: status bar + top nav (103px, always on top) ── */}
      ...
      {/* ── OVERLAY: search + brand switcher, eases in on scroll-up (112px) ── */}
      <div
        style={{
          position: "absolute",
          top: 103,
      ...
      >
        <SearchBar />
        <SportBrandSwitcher onONClick={goToON} />
      </div>

      {/* ── SCROLLABLE AREA (starts at 103, search+switcher are first children) ── */}
      <div
        ref={scrollRef}
      ...
      >
        {/* Sentinel: observer watches this; placed at very top before inline header */}
        ...
        {/* Horizontal scroll sport hero cards */}
        <div
          style={{
            overflowX: "auto",
            overflowY: "hidden",
        ...
        >
          {HERO_CARDS.map((card) => (
            <div key={card.label} style={{ flexShrink: 0, width: 140, cursor: "pointer", display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, color: NAVY, lineHeight: "14px", letterSpacing: 0.1, display: "block" }}>
                {card.label}
              </span>
            </div>
          ))}
        ...
        >
          {CATEGORY_CARDS.map((card) => (
            <div key={card.label} onClick={card.label === "Shop All" ? goToShopAll : undefined} style={{ cursor: "pointer", display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: NAVY, lineHeight: "16px", letterSpacing: 0.12, display: "block" }}>
                {card.label}
              </span>
            </div>
          ))}
